...
@Path("/getStockPrices2")
public class GetStockHistory2 {
	@GET
	@Path("{stocksymbols}/{api_token}/{date_from}/{date_to}")
	@Produces(MediaType.APPLICATION_JSON)
	public Response getStockPrices(@PathParam("stocksymbols") String stocksymbols,
			@PathParam("api_token") String api_token, @PathParam("date_from") String date_from,
			@PathParam("date_to") String date_to) {

		// System.out.println("Getting .... stock quotes for " + stocksymbols);
		String[] stockSymbolarray = { "adbe", "msft", "orcl" };
		List<StockPrices> stockPricesList = new ArrayList();
		for (int i = 0; i < stockSymbolarray.length; i++) {
			String stocksymbol = stockSymbolarray[i];
